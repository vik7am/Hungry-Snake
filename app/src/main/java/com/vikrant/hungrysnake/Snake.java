...
public class Snake{
    ...
    Snake(GameData gameData,Egg egg) {
        ...
        SIZE=gameData.SIZE;
        X=new ArrayList<Integer>(100);
        Y=new ArrayList<Integer>(100);
        X.add(SIZE);Y.add(0);
        X.add(0);Y.add(0);
    ...
    public void move(boolean NEW) {
        ...
        X.add(0, X.get(0)+x);
        Y.add(0, Y.get(0)+y);
        if(X.get(0)==WIDTH) X.set(0, 0);
        if(Y.get(0)==HEIGHT) Y.set(0, 0);
        if(X.get(0)==-SIZE) X.set(0, WIDTH-SIZE);
        if(Y.get(0)==-SIZE) Y.set(0, HEIGHT-SIZE);
